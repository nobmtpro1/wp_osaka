...
<div class="sd-div sd-setting" id="sd-general-settings">
    ...
    <p class="description"><?php esc_html_e('API Key nhận được sau khi đăng ký tài khoản với ShipDepot, có thể xem thêm hướng dẫn tại đây', 'ship-depot-translate') ?> <a href="<?php echo esc_url(SHIP_DEPOT_SITE) ?>/how-to-get-api-key" target="_blank"><?php echo esc_url(SHIP_DEPOT_SITE) ?>/how-to-get-api-key</a></p>
    <?php
    echo '<input name="sd_api_key" id="sd_api_key" type="text" value="' . esc_attr(get_option('sd_api_key')) . '">'
    ?>

    <b>Webhook</b>
    <p class="description"><?php esc_html_e('Copy địa chỉ webhook bên dưới rồi cập nhật vào tài khoản của bạn ở ShipDepot để đảm bảo trạng thái vận đơn được cập nhật từ đơn vị vận chuyển sẽ gửi đến bạn.', 'ship-depot-translate') ?></p>
    <?php
    echo '<a href="#" target="_blank">' . esc_url(get_site_url()) . '/wp-json/shipdepot/webhook</a>';
    ?>

    <b><?php esc_html_e('Thông tin người gửi (chủ shop / chủ cửa hàng)', 'ship-depot-translate') ?></b>
    <p class="description"><?php esc_html_e('Thông tin này được in trên phiếu gửi hàng và được thiết lập bởi ShipDepot khi bạn đăng ký tài khoản, bạn cần liên hệ ShipDepot để thay đổi. Xem thêm tại', 'ship-depot-translate') ?> <a href="<?php echo esc_url(SHIP_DEPOT_SITE) ?>/how-to-change-sender-info" target="_blank"><?php echo esc_url(SHIP_DEPOT_SITE) ?>/how-to-change-sender-info</a></p>
    <div id="sender-info">
        <?php
    ...
    <p class="description"><?php esc_html_e('Thay đổi hoặc bổ sung kho hàng cần thực hiện tại ShipDepot. Xem thêm tại', 'ship-depot-translate') ?> <a href="<?php echo esc_url(SHIP_DEPOT_SITE) ?>/how-to-update-warehouse" target="_blank"><?php echo esc_url(SHIP_DEPOT_SITE) ?>/how-to-update-warehouse</a></p>
    ...
    <div class="container-fluid">
        <div class="row<?php echo get_option('sd_auto_create_shipping') == 'yes' ? '' : ' disable-element' ?>" id="div_auto_shipping">
            ...
            foreach ($listStatus as $id => $stt) {
                ...
                $checkedAttr = checked(get_option('sd_status_auto_create_shipping'), $id, false);
                echo '<label for="' . esc_attr($id) . '" class="label-status"><input name="sd_status_auto_create_shipping" id="' . esc_attr($id) . '" type="radio" value="' . esc_attr($id) . '" ' . $checkedAttr . '  />' . esc_html($stt) . '</label>';
                echo '</div>';
                $count++;
            }
            ?>
