...
if (!function_exists('sd_add_meta_boxes')) {
    function sd_add_meta_boxes()
    {
        add_meta_box('sd_meta_boxes', __('Vận đơn - ShipDepot', 'ship-depot-translate'), 'sd_meta_boxes_content', 'shop_order', 'normal', 'core');
    }
}
...
if (!function_exists('sd_save_wc_order_other_fields')) {
    ...
    {
        Ship_Depot_Logger::wrlog('[sd_save_wc_order_other_fields]');
        ...
        Ship_Depot_Logger::wrlog('[sd_save_wc_order_other_fields] post_type: ' . sanitize_key($_POST['post_type']));
        Ship_Depot_Logger::wrlog('[sd_save_wc_order_other_fields] action: ' . sanitize_key($_POST['action']));
        if ('page' == sanitize_key($_POST['post_type'])) {
...
function sd_edit_cod_amount($order_id)
{
    ...
    if (isset($_POST['shipdepot']['is_cod_edit']) && !Ship_Depot_Helper::check_null_or_empty(sanitize_text_field($_POST['shipdepot']['is_cod_edit'])) && sanitize_text_field($_POST['shipdepot']['is_cod_edit']) == 'true') {
        ...
        $ship_info = json_decode(get_post_meta($order_id, 'sd_ship_info', true));
        ...
        //
        $selected_courier = get_post_meta($order_id, 'sd_selected_courier', true);
        ...
        if ($rs->Code > 0) {
            ...
            if ($result) {
                ...
                $str_cod = get_post_meta($order_id, 'sd_cod', true);
                if (!Ship_Depot_Helper::check_null_or_empty($str_cod)) {
                    $cod = json_decode($str_cod);
                    $old_cod = $cod->Value;
...
function sd_woocommerce_new_shipping($order_id)
{
    ...
    update_post_meta($order_id, 'sd_not_create_ship', json_encode($not_create_ship));
    sd_submit_data_and_save_to_order_meta_data($order_id, false, $not_create_ship);
}

function sd_submit_data_and_save_to_order_meta_data($order_id, $is_edit, $save_only)
{
    $need_call_update = false;
    //Check api key first
    $shop_api_key = get_option('sd_api_key');
    if (Ship_Depot_Helper::check_null_or_empty($shop_api_key)) {
        Ship_Depot_Logger::wrlog('[sd_submit_data_and_save_to_order_meta_data] API Key is empty => cannot save ');
        return false;
    }
    $get_data = new Ship_Depot_Get_Data($_POST, $order_id);
    ...
    if ($is_edit) {
        $need_call_update = compare_to_order_meta_and_save($order_id, 'sd_list_package_size', $json_packages);
    } else {
    ...
    if ($is_edit) {
        $need_call_update = compare_to_order_meta_and_save($order_id, 'sd_sender_storage', $sender_storage);
    } else {
        update_post_meta($order_id, 'sd_sender_storage', $sender_storage);
    }
    ...
    if ($is_edit) {
        $need_call_update = compare_to_order_meta_and_save($order_id, 'sd_receiver', $json_rcv);
    } else {
    ...
    if ($is_edit) {
        $need_call_update = compare_to_order_meta_and_save($order_id, 'sd_insurance', $json_insr);
    } else {
    ...
    $json_cod = json_encode($cod);
    if ($is_edit) {
        $need_call_update = compare_to_order_meta_and_save($order_id, 'sd_cod', $json_cod);
    } else {
        update_post_meta($order_id, 'sd_cod', $json_cod);
    }
    ...
    if ($is_edit) {
        $need_call_update = compare_to_order_meta_and_save($order_id, 'sd_selected_shipping', $json_selected_shipping);
    } else {
        update_post_meta($order_id, 'sd_selected_shipping', $json_selected_shipping);
    }

    //selected_courier
    $selected_courier = $get_data->get_selected_courier();
    if ($is_edit) {
        $need_call_update = compare_to_order_meta_and_save($order_id, 'sd_selected_courier', $selected_courier);
    } else {
        update_post_meta($order_id, 'sd_selected_courier', $selected_courier);
    }
    ...
    if ($selected_courier == GHN_COURIER_CODE) {
        ...
        update_post_meta($order_id, 'sd_ship_from_station', json_encode($ship_from_station, JSON_UNESCAPED_UNICODE));
    } else {
        update_post_meta($order_id, 'sd_ship_from_station', '');
    }
    ...
    if ($is_edit) {
        $need_call_update = compare_to_order_meta_and_save($order_id, 'sd_shipping_notes', $shipping_notes);
    } else {
    ...
    update_post_meta($order_id, 'sd_customer_pay_shipping', json_encode($cus_pay_ship));
    ...
    if ($is_edit) {
        $need_call_update = compare_to_order_meta_and_save($order_id, 'sd_list_items', $json_items);
    } else {
        update_post_meta($order_id, 'sd_list_items', $json_items);
    }
    ...
    if (!$save_only) {
        ...
        if (!$is_edit || $need_call_update) {
            ...
            $data_input = array(
                "order_id" => $order_id,
                "order_created_date" => Ship_Depot_Helper::ParseObjectToArray($order->get_date_created()), //Order date need parse to array because WC_DateTime object cannot send to API
            ...
            if ($rs->Code > 0) {
                if (!$is_edit) {
                    if (Ship_Depot_Helper::check_null_or_empty($rs->Data->TrackingNumber)) {
                        ...
                        update_post_meta($order_id, 'sd_ship_info', json_encode($rs->Data, JSON_UNESCAPED_UNICODE));
                    }
                }
            } else {
                $order_note = __('Vận đơn tạo thất bại. Lý do:', 'ship-depot-translate');
...
function compare_to_order_meta_and_save($order_id, $order_meta_key, $data): bool
{
    $meta_data = get_post_meta($order_id, $order_meta_key);
    if (Ship_Depot_Helper::CompareData($meta_data, $data)) {
        return true;
        update_post_meta($order_id, 'order_meta_key', $data);
    }
    return false;
}
...
function metabox_order($order)
{
    ...
    if (!$order) {
        ...
        $array_side_box = ["woocommerce-shipment-dhl-label,woocommerce-order-actions,woocommerce-order-notes"];
...
?>
    ...
    <tr id="cod_price_content">
        ...
        <td>
            <div id="cod_price">
                <input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[advance][cod][value]" id="tb_cod_amount" class="no-spin" />
    ...
    <?php
    $is_edit = get_post_meta($order_id, 'sd_is_edit_order', true);
    $str_ship_info = get_post_meta($order_id, 'sd_ship_info', true);
    $idxShipping = 1;
    ...
    <script type="text/javascript">
        jQuery(document).ready(function($) {
            ...
            let cod_amount = parseFloat(<?php echo esc_js(wc_get_order($order_id)->get_subtotal()) ?>);
...
function cancel_shipping_init()
{
    ...
    $data_input = array();
    $ship_info = json_decode(get_post_meta($order_id, 'sd_ship_info', true));
    Ship_Depot_Logger::wrlog('[cancel_shipping_init] ship_info: ' . print_r($ship_info, true));
    $data_input["tracking_number"] = isset($ship_info->TrackingNumber) ? $ship_info->TrackingNumber : '';
    //
    $selected_courier = get_post_meta($order_id, 'sd_selected_courier', true);
    $data_input["selected_courier"] = $selected_courier;
    Ship_Depot_Logger::wrlog('[cancel_shipping_init] data_input: ' . print_r($data_input, true));
    if (Ship_Depot_Helper::check_null_or_empty($data_input["tracking_number"])) {
        wp_send_json_success(__('Đơn hàng chưa có vận đơn.', 'ship-depot-translate'));
        die();
    }

    if (Ship_Depot_Helper::check_null_or_empty($data_input["selected_courier"])) {
        wp_send_json_success(__('Không xác định được đơn vị vận chuyển để hủy.', 'ship-depot-translate'));
        die();
    }

    $url = SHIP_DEPOT_HOST_API . '/Shipping/CancelShipping';
    $rs = Ship_Depot_Helper::http_post($url, $data_input, array('ShopAPIKey' => $shop_api_key));
    Ship_Depot_Logger::wrlog('[cancel_shipping_init] result call API: ' . print_r($rs, true));
    if ($rs->Code > 0) {
        $result = $rs->Data;
        Ship_Depot_Logger::wrlog('[cancel_shipping_init] result: ' . print_r($result, true));
        if ($result) {
            //Remove shipping info
            update_post_meta($order_id, 'sd_ship_info', '');
            $order_note = __('Hủy vận đơn [Tracking_number] thành công.', 'ship-depot-translate');
        } else {
            $order_note = __('Hủy vận đơn [Tracking_number] thất bại. Lý do:', 'ship-depot-translate');
            $order_note = $order_note . ' ' . $rs->Msg;
        }
    } else {
        $order_note = __('Hủy vận đơn [Tracking_number] thất bại. Lý do:', 'ship-depot-translate');
...
{
    //return [];
    return ['sd_not_create_ship', 'sd_list_package_size', 'sd_sender_storage', 'sd_sender_info', 'sd_ship_info', 'sd_receiver', 'sd_insurance', 'sd_cod', 'sd_selected_shipping', 'sd_selected_courier', 'sd_shipping_notes', 'sd_customer_pay_shipping', 'sd_is_edit_order', 'sd_list_items', 'sd_cod_failed_info'];
}
...
if (!function_exists('sd_meta_boxes_content')) {
    function sd_meta_boxes_content()
    {
        ...
        Ship_Depot_Logger::wrlog('[sd_meta_boxes_content] post_meta_data: ' . print_r($post_meta_data, true));
        ...
        $not_create_ship = get_post_meta($post->ID, 'sd_not_create_ship', true);
        $str_ship_info = get_post_meta($post->ID, 'sd_ship_info', true);
        $is_add_new = false;
        ...
        if (!Ship_Depot_Helper::check_null_or_empty($str_list_couriers)) {
            ...
            foreach ($listCouriers as $courier_obj) {
                ...
                if ($courier->CourierID == get_post_meta($post->ID, 'sd_selected_courier', true)) {
        ...
        $json_shipping = get_post_meta($post->ID, 'sd_selected_shipping', true);
        ...
        if (!Ship_Depot_Helper::check_null_or_empty($json_shipping)) {
            ...
            set_shipping($post->ID, ($selected_shipping->ShopMarkupShippingFee->IsActive ? $selected_shipping->ShopMarkupShippingFee->ShippingFeeTotal : $selected_shipping->ShipDepotMarkupShippingFee->ShippingFeeTotal) + $selected_shipping->NoMarkupShippingFee->NoMarkupShippingFeeTotal);
        }
        ...
        $json_list_package_size = get_post_meta($post->ID, 'sd_list_package_size', true);
        $total_weight = 0;
        ...
        if ($is_add_new) {
            ...
            $str_sender_info = get_post_meta($post->ID, 'sd_sender_info', true);
        }
        ...
        if (!Ship_Depot_Helper::check_null_or_empty($str_list_storages) && !Ship_Depot_Helper::check_null_or_empty(get_post_meta($post->ID, 'sd_sender_storage', true))) {
            $list_storages = Ship_Depot_Helper::CleanJsonFromHTMLAndDecode($str_list_storages);
            foreach ($list_storages as $str) {
                if ($str->WarehouseID == get_post_meta($post->ID, 'sd_sender_storage', true)) {
                    $storage = $str;
                }
            }
        }
        //
        $str_rcv = get_post_meta($post->ID, 'sd_receiver', true);
        ...
        //
        $str_cod = get_post_meta($post->ID, 'sd_cod', true);
        if (!Ship_Depot_Helper::check_null_or_empty($str_cod)) {
            $cod = new Ship_Depot_Insurance(Ship_Depot_Helper::CleanJsonFromHTMLAndDecode($str_cod));
        }
        //
        //Tạm thời ko cho sửa vận đơn
        $str_insurance = get_post_meta($post->ID, 'sd_insurance', true);
        ...
        //
        $notes = get_post_meta($post->ID, 'sd_shipping_notes', true);
        //
        $str_cod_failed_info = get_post_meta($post->ID, 'sd_cod_failed_info', true);
        ...
        $json_ship_fr_station = get_post_meta($post->ID, 'sd_ship_from_station', true);
    ...
    ?>
            ...
            jQuery(document).ready(function($) {
                if ($('button[type=submit][name=save]').length > 0) {
                    $('button[type=submit][name=save]').click(function(event) {
                        ...
                        if ($is_add_new) {
                        ?>
                            if ($('#cb_not_create_shipping').is(':checked') == false) {
                                if (validateData('<?php echo esc_js($post->ID) ?>')) {
                                    if ($('#selectedShipping').val() == '' || $('input[class=radio_shipping_fee]:checked').length == 0) {
                                        msgHtml = `<?php esc_html_e('Yêu cầu chọn loại vận chuyển trước khi lưu.', 'ship-depot-translate') ?>`;
                                        $('#error_message_content').html(msgHtml);
                                        $('html, body').animate({
                                            scrollTop: $("#shipping_fee").offset().top
                                        }, 0);
                                        return;
                                    } else {
                                    ...
                                    $('html, body').animate({
                                        scrollTop: $("#shipping_fee").offset().top - 300
                                    }, 0);
                                    return;
                                }
                            }
                        ...
                        validateStationData()
                        if (!isNullorEmpty($('#error_station_content').text())) {
                            $('html, body').animate({
                                scrollTop: $("#shipping_fee").offset().top
                            }, 0);
                            return;
                        ...
                        //Logic GHTK
                        let selectedCour = $('#selectedCourier').val();
                        if (selectedCour == '<?php echo esc_js(GHTK_COURIER_CODE) ?>') {
                            let type = $('input[class=rd_receiver_type]:checked').val();
                            let cusProvince = '';
                            let cusProvinceText = '';
                            let cusDist = '';
                            let cusWard = '';
                            let cusAddr = '';
                            if (type == 'other') {
                                cusProvince = $('#sl_receiver_province').val();
                                cusProvinceText = $('#sl_receiver_province option:selected').text();
                                cusDist = $('#sl_receiver_district').val();
                                cusWard = $('#sl_receiver_ward').val();
                                cusAddr = $('#receiver_address').val();
                            } else if (type == 'current') {
                                cusProvince = $('#_shipping_city').val();
                                cusProvinceText = $('#_shipping_city option:selected').text();
                                cusDist = $('#_shipping_district').val();
                                cusWard = $('#_shipping_ward').val();
                                cusAddr = $('#_shipping_address_1').val();
                            }
                            let strProNeedHamlet = '<?php echo esc_js(GHTK_PROVINCE_SPECIAL) ?>';
                            const provinceNeedHamlet = strProNeedHamlet.split(",");
                            if (provinceNeedHamlet.includes(cusProvince)) {
                                if ($("#slGHTKHamlet").length > 0) {
                                    $("#slGHTKHamlet").empty();
                                    let curHamlet = $("#ghtkHamlet").val();
                                    let isSameAddr = false;
                                    if (curHamlet != '') {
                                        if (cusProvince == '<?php echo isset($receiver) ? esc_js($receiver->Province) : '' ?>' && cusDist == '<?php echo isset($receiver) ? esc_js($receiver->District) : '' ?>' && cusWard == '<?php echo isset($receiver) ? esc_js($receiver->Ward) : '' ?>' && cusAddr == '<?php echo isset($receiver) ? esc_js($receiver->Address) : '' ?>') {
                                            isSameAddr = true;
                                        }
                                    }
                                    let dataInput = {
                                    ...
                                    jQuery.ajax({
                                        url: '<?php echo esc_url(SHIP_DEPOT_HOST_API) ?>' + '/Shipping/GetHamlet',
                                        headers: {
        ...
        <input type="hidden" id="order_id" value="<?php echo esc_attr($post->ID) ?>" />
        <?php
        if (!$is_add_new) {
        ?>
                                ...
                                <div class="col-xl-2 col-lg-6 col-md-6 col-sm-6 col-12 col-padding-0">
                                    ...
                                    <span id="cod_edit_area" style="display: none;">
                                        <div id="div_cod_edit_amount">
                                            ...
                                            <input id="tb_cod_edit_amount" name="shipdepot[cod_edit_amount]" pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" class="no-spin" value="<?php echo esc_attr(Ship_Depot_Helper::currency_format($cod->Value, '')) ?>" />
                                            <p class="description">₫</p>
                                        </div>
                    ...
                    <div id="shipping_info" <?php echo $not_create_ship == 'true' ? 'style="display:none"' : '' ?>>
                                    ...
                                    ?>
                                            ...
                                            <div class="container-fluid col-padding-0">
                                                <div class="row">
                                                    ...
                                                    <div class="col-xl-2 col-lg-3 col-md-3 col-sm-3 col-3">
                                                        <input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[pk_df][length]" class="package_length no-spin" id="pk_df_package_length" value="<?php echo esc_attr(Ship_Depot_Helper::number_format($pk_df->Length)) ?>" />
                                                    </div>
                                                    <div class="col-xl-2 col-lg-3 col-md-3 col-sm-3 col-3">
                                                        <input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[pk_df][width]" class="package_width no-spin" id="pk_df_package_width" value="<?php echo esc_attr(Ship_Depot_Helper::number_format($pk_df->Width)) ?>" />
                                                    </div>
                                                    <div class="col-xl-2 col-lg-3 col-md-3 col-sm-3 col-3">
                                                        <input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[pk_df][height]" class="package_height no-spin" id="pk_df_package_height" value="<?php echo esc_attr(Ship_Depot_Helper::number_format($pk_df->Height)) ?>" />
                                                    </div>
                                                    <div class="col-xl-2 col-lg-3 col-md-3 col-sm-3 col-3">
                                                        <input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[pk_df][weight]" class="package_weight no-spin" id="pk_df_package_weight" value="<?php echo esc_attr(Ship_Depot_Helper::number_format($pk_df->Weight)) ?>" />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                                ...
                                                <div class="package_size_row">
                                                    ...
                                                    <div class="container-fluid col-padding-0">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-xl-2 col-lg-3 col-md-3 col-sm-3 col-3">
                                                                <input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[<?php echo esc_attr($pk_id) ?>][length]" class="package_length no-spin" id="<?php echo esc_attr($pk_id) ?>_package_length" value="<?php echo esc_attr(Ship_Depot_Helper::number_format($pk_size->Length)) ?>" />
                                                            </div>
                                                            <div class="col-xl-2 col-lg-3 col-md-3 col-sm-3 col-3">
                                                                <input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[<?php echo esc_attr($pk_id) ?>][width]" class="package_width no-spin" id="<?php echo esc_attr($pk_id) ?>_package_width" value="<?php echo esc_attr(Ship_Depot_Helper::number_format($pk_size->Width)) ?>" />
                                                            </div>
                                                            <div class="col-xl-2 col-lg-3 col-md-3 col-sm-3 col-3">
                                                                <input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[<?php echo esc_attr($pk_id) ?>][height]" class="package_height no-spin" id="<?php echo esc_attr($pk_id) ?>_package_height" value="<?php echo esc_attr(Ship_Depot_Helper::number_format($pk_size->Height)) ?>" />
                                                            </div>
                                                            <div class="col-xl-2 col-lg-3 col-md-3 col-sm-3 col-3">
                                                                <input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[<?php echo esc_attr($pk_id) ?>][weight]" class="package_weight no-spin" id="<?php echo esc_attr($pk_id) ?>_package_weight" value="<?php echo esc_attr(Ship_Depot_Helper::number_format($pk_size->Weight)) ?>" />
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="package_size_row">
                                            ...
                                            <div class="container-fluid col-padding-0">
                                                <div class="row">
                                                    <input type="hidden" class="shipdepot_package" name="shipdepot[package_id][]" value="pk_df" />
                                                    <div class="col-xl-2 col-lg-3 col-md-3 col-sm-3 col-3"><input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[pk_df][length]" class="package_length no-spin" id="pk_df_package_length" /></div>
                                                    <div class="col-xl-2 col-lg-3 col-md-3 col-sm-3 col-3"><input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[pk_df][width]" class="package_width no-spin" id="pk_df_package_width" /></div>
                                                    <div class="col-xl-2 col-lg-3 col-md-3 col-sm-3 col-3"><input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[pk_df][height]" class="package_height no-spin" id="pk_df_package_height" /></div>
                                                    <div class="col-xl-2 col-lg-3 col-md-3 col-sm-3 col-3"><input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[pk_df][weight]" class="package_weight no-spin" id="pk_df_package_weight" /></div>
                                                </div>
                                            </div>
                                        </div>
                                    <?php
                        ...
                        <div id="pick-station" class="div-pick-station shipping-info-property">
                            ...
                            <div class="container-fluid col-padding-0 pick-station-options">
                                <div class="row">
                                    <div class="col-xl-2 col-lg-4 col-md-8 col-sm-10 div-ship-from-station-no">
                                    ...
                                    <div class="col-xl-4 col-lg-8 col-md-8 col-sm-10 pick-station-yes">
                                        ...
                                        <div class="pick-station-yes-option" style="<?php echo $ship_from_station->IsActive ? esc_attr('display: block') : 'display: none' ?>">
                                            <div class="pick-station-yes-option-row">
                                                <label class="pick-station-yes-title"><?php esc_html_e('Tỉnh/Thành', 'ship-depot-translate') ?></label>
                                            ...
                                            <div class="pick-station-yes-option-row">
                                                <label class="pick-station-yes-title"><?php esc_html_e('Quận/Huyện', 'ship-depot-translate') ?></label>
                                ...
                                <div class="row" id="sd-advance-row">
                                    <div class="col-xl-3 col-lg-6 col-md-6 col-sm-6 col-12 col-padding-0 insurance-detail-col">
                                        ...
                                        <div style="<?php echo $display_insr_price ? '' : 'display: none;' ?>" id="insr_price">
                                            <input pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" data-type="currency" type="text" name="shipdepot[advance][insurance][value]" id="tb_ins_fee" class="no-spin" value="<?php echo isset($insurance) ? esc_attr(Ship_Depot_Helper::currency_format(floatval($insurance->Value), ""))  : '0' ?>" />
                                            <p class="description">₫</p>
                                        </div>
                                    </div>
                                    <div class="col-xl-3 col-lg-6 col-md-6 col-sm-6 col-12 col-padding-0">
                        ...
                        <div id="shipping_fee" class="shipping-info-property">
                            ...
                            <input type="hidden" id="selectedCourier" name="shipdepot[selectedCourier]" value="<?php echo esc_attr(get_post_meta($post->ID, 'sd_selected_courier', true)) ?>" />
                            ...
                            <div id="shipping_fee_content">
                                ...
                                if (isset($list_shipping) && $list_shipping != false) {
                                    foreach ($list_shipping as $shipping) {
                                        $isEnabled = $selected_courier->CourierID == $shipping->CourierID;
                                ?>

                                        <div id="<?php echo esc_attr($shipping->CourierID) ?>" class="courier-fee<?php echo $isEnabled ? '' : ' disable-element' ?>">
                                            <img src="<?php echo esc_url($shipping->LogoURL) ?>" alt="<?php echo esc_attr($shipping->CourierName) ?>" data-placement="bottom" title="<?php echo esc_attr($shipping->CourierName) ?>">
                                            <div class="service-fee">
                                                <?php foreach ($shipping->ListServices as $serv) {
                                                ...
                                                ?>
                                                    <div>
                                                        ...
                                                        <p class="description">T.gian nhận hàng ước tính: <?php echo esc_html($serv->TimeExpected) ?></p>
                                                    </div>
                                                <?php
                                                }
                                                ?>
