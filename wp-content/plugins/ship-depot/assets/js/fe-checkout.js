jQuery(document).ready(function ($) {
        ...
        if ($('#billing_city').length > 0) {
            ...
            if (checkNullorEmpty(district_code)) {
                ...
                if (isDTExisted && isWardExisted) {
                    ...
                    html = '<option>' + sd_fe_checkout_params.l10n.select_ward + '</option>';
                    allProvinces.forEach(pro => {
                        if (pro.Code == province_code) {
                            pro.ListDistricts.forEach(dis => {
                                if (dis.Code == district_code) {
    ...
    function ResetBillingDTandW(province_code) {
        ...
        html = '<option>' + sd_fe_checkout_params.l10n.select_district + '</option>';
        allProvinces.forEach(pro => {
            if (pro.Code == province_code) {
                pro.ListDistricts.forEach(dis => {
    ...
    if ($('#order_review').length > 0) {
        $('#order_review').on('change', 'input[type=radio][class=radio_shipping_fee]', function () {
            $(document.body).trigger('update_checkout');
        });

        $('#order_review').on('change', 'input[type=radio][name=payment_method]', function () {
            $(document.body).trigger('update_checkout');
        });

        $('#order_review').on('click', '#place_order', function (event) {
        ...
        if ($('#myModal').length > 0) {
            if ($('#btnModalOK').length > 0) {
                $('#order_review').on('click', '#btnModalOK', function () {
        ...
        $('#order_review').on('change', 'textarea[name=shipdepot_shipping_notes]', function () {
