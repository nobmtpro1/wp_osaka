...
class Ship_Depot_Order_Shipping
{
     ...
     */
    public static function calculate_shipping_fee($is_cod, $is_insurance, $insurance_amount, $list_packages, $list_items, $receiver, $sender_storage, $sender_info, $items_total_price, $item_regular_price_total, $for_courier = '')
    {
        ...
        $cod->IsActive = $is_cod;
        $cod->Value = $is_cod ? $items_total_price : 0;
...
add_action('woocommerce_order_status_changed', 'sd_create_shipping_auto', 10, 3);

function sd_create_shipping_auto($order_id, $old_status, $new_status)
{
    $order = wc_get_order($order_id);
    Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] order_id: ' . print_r($order_id, true));
    Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] _POST: ' . print_r($_POST, true));
    Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] old_status: ' . print_r($old_status, true));
    Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] new_status: ' . print_r($new_status, true));
    Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] post_type: ' . sanitize_text_field($_POST['post_type']));
    Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] action: ' . sanitize_text_field($_POST['action']));
    if (sanitize_key($_POST['post_type']) == 'shop_order' && sanitize_key($_POST['action']) == 'editpost') {
        Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] from edit order => Not create shipping auto ');
        return;
    }
    /*
    pending
    ...
    */
    ...
    Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] auto_create_ship: ' . $auto_create_ship);
    if ($auto_create_ship == 'yes' || $auto_create_ship == '1') {
        $stt_auto_create_ship = get_option('sd_status_auto_create_shipping');
        Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] stt_auto_create_ship: ' . $stt_auto_create_ship);
        if ('wc-' . $new_status == $stt_auto_create_ship) {
            $str_ship_info = get_post_meta($order_id, 'sd_ship_info', true);
            // $not_create_ship = get_post_meta($order_id, 'sd_not_create_ship', true);
            // if (Ship_Depot_Helper::check_null_or_empty($not_create_ship) || $not_create_ship != 'true') {

            // } else {
            //     Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] not_create_ship = ' . $not_create_ship . ' => Not create shipping');
            // }
            ...
            if (!Ship_Depot_Helper::check_null_or_empty($str_ship_info)) {
                ...
                if (Ship_Depot_Helper::check_null_or_empty($ship_info->TrackingNumber)) {
                    Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] TrackingNumber empty => Create shipping');
                    save_shipping_to_order_meta_data($order_id);
                    $rs = create_ship($order_id);
                    if ($rs) {
                        update_post_meta($order_id, 'sd_not_create_ship', json_encode(false));
                    }
                } else {
                    Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] TrackingNumber existed: ' . $ship_info->TrackingNumber . ' => Not create shipping');
                }
            } else {
                Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] str_ship_info = null => Create shipping');
                save_shipping_to_order_meta_data($order_id);
                $rs = create_ship($order_id);
                if ($rs) {
                    update_post_meta($order_id, 'sd_not_create_ship', json_encode(false));
            ...
            Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] order_status # stt_auto_create_ship => Not create shipping');
        }
    } else {
        Ship_Depot_Logger::wrlog('[sd_create_shipping_auto] auto_create_ship = false => Not create shipping ');
    }
}
...
function create_ship($order_id)
{
    Ship_Depot_Logger::wrlog('[create_ship] get_post_meta: ' . print_r(get_post_meta($order_id, '', true), true));
    //List package sizes
    $list_package_size = [];
    $json_list_package_size = get_post_meta($order_id, 'sd_list_package_size', true);
    ...
    //Sender
    $json_sender_info = get_post_meta($order_id, 'sd_sender_info', true);
    ...
    //Sender storage
    $sender_storage = get_post_meta($order_id, 'sd_sender_storage', true);
    Ship_Depot_Logger::wrlog('[create_ship] sender_storage: ' . $sender_storage);
    //Receiver
    $json_rcv = get_post_meta($order_id, 'sd_receiver', true);
    ...
    //Insurance
    $json_insurance = get_post_meta($order_id, 'sd_insurance', true);
    ...
    //Cod
    $json_cod = get_post_meta($order_id, 'sd_cod', true);
    ...
    //Selected Shipping
    $json_selected_shipping = get_post_meta($order_id, 'sd_selected_shipping', true);
    ...
    //Shipping notes
    $shipping_notes = get_post_meta($order_id, 'sd_shipping_notes', true);
    Ship_Depot_Logger::wrlog('[create_ship] shipping_notes: ' . $shipping_notes);
    //Customer pay shipping
    $str_cus_pay_ship = get_post_meta($order_id, 'sd_customer_pay_shipping', true);
    ...
    //cod failed info
    $json_cod_failed_info = get_post_meta($order_id, 'sd_cod_failed_info', true);
