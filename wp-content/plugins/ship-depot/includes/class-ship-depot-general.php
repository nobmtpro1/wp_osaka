...
?>
        <div class="notice notice-error is-dismissible">
            <p><?php printf(
                    __('%s is enabled but not effective. It requires %s in order to work.', 'ship-depot-translate'),
                    '<strong>' . esc_html__('FriendStore for WooCommerce', 'ship-depot-translate') . '</strong>',
    ...
    ?>
        <div class="vf-notice notice notice-error is-dismissible">
            <p><?php printf(
                    __('%s was auto deactivated. It requires %s in order to work properly.', 'ship-depot-translate'),
                    '<strong>' . esc_html__('FriendStore for WooCommerce', 'ship-depot-translate') . '</strong>',
                    '<a href="https://wordpress.org/plugins/woocommerce/" target="_blank"><strong>WooCommerce</strong></a>'
                ); ?></p>
        </div>
<?php
    ...
    public function auto_deactivate()
    {
        ...
        if (is_multisite()) {
            ...
            $fl_array = preg_grep("/ship-depot-translate.php$/", array_keys($plugins));
            $vf_plugin = reset($fl_array);
        } else {
            $plugins = apply_filters('active_plugins', get_option('active_plugins', array()));
            $fl_array = preg_grep("/ship-depot-translate.php$/", $plugins);
            if (count($fl_array) > 0) {
