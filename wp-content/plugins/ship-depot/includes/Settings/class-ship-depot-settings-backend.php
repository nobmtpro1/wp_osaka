    ...
    class Ship_Depot_Settings extends WC_Settings_Page
    {
        ...
        public function output_sections()
        {
            ...
            foreach ($sections as $id => $label) {
                echo '<li class="' . esc_attr(($current_section == $id ? 'selected-section' : '')) . ' ' . esc_attr('section-setting') . '"><a href="' . esc_url(admin_url('admin.php?page=wc-settings&tab=' . $this->id . '&section=' . sanitize_title($id))) . '" class="' . esc_attr(($current_section == $id ? 'current' : '')) . '">' . esc_html($label) . '</a></li>';
            }
            ...
            if (!isset($_POST['sd_fee_modify_nonce']) || !wp_verify_nonce($_POST['sd_fee_modify_nonce'], 'sd_fee_modify')) {
                ...
                if (isset($_POST['fee_modify']['fees']['id'])) {
                    ...
                    foreach ($_POST['fee_modify']['fees']['id'] as $key => $id) {
                        ...
                        Ship_Depot_Logger::wrlog('[save_fee_modify] ' . $fee_index_id . ' data: ' . print_r(sanitize_text_field($_POST[$fee_index_id]), true));
                        //Order conditions
                        ...
                        if (isset($_POST[$fee_index_id]['if'])) {
                            foreach ($_POST[$fee_index_id]['if'] as $index => $id) {
                                ...
                                Ship_Depot_Logger::wrlog('[save_fee_modify] ' . $if_id . ' data: ' . print_r(sanitize_text_field($_POST[$fee_index_id][$if_id]), true));
